...
- 根据顶部的"胜利"/"失败"文字判断左侧队伍的比赛结果
- 每名选手旁边会显示英雄名称、KDA（击杀/死亡/助攻）、评分（如 10.4）等信息
- 选手的分路位置需要根据英雄和位置来判断：
  · 对抗路（上单/边路）：常见英雄如吕布、关羽、夏侯惇、花木兰、芈月等
  · 打野：常见英雄如赵云、韩信、李白、露娜、澜等
  · 中路（法师）：常见英雄如妲己、王昭君、诸葛亮、司马懿等
  · 发育路（射手）：常见英雄如后羿、鲁班、马可波罗、公孙离等
  · 游走（辅助）：常见英雄如蔡文姬、鬼谷子、张飞、瑶等

请提取以下信息：

1. **比赛结果**：胜利 或 失败（以左侧队伍为准）
2. **比赛日期**：格式 YYYY/M/D（如 2026/2/6），没有则返回 null
3. **左侧队伍5名选手**：每人包括游戏ID、分路位置、评分
4. **MVP**：获得 MVP 的选手游戏 ID 和评分（MVP 图标通常标注在某个选手旁边）
5. **奖牌**：识别每个选手的奖牌等级（顶级/金牌/银牌/铜牌，看选手旁边的奖牌图标和文字）

注意事项：
- 请仔细辨认每个汉字和特殊符号，游戏 ID 中可能包含丶、丨、★、ω、℃ 等特殊符号
- 评分是选手名字下方或旁边的数字（如 10.4、7.9、5.6）
- 分路位置如果无法确定请填 "未知"
- 奖牌等级看选手旁边的图标/文字：金牌打野、银牌对抗等
- 只返回 JSON，不要有其他文字说明
...
返回格式：
{
    ...
    "选手": [
        ...
        {"游戏ID": "yyy", "分路": "打野", "评分": 6.3, "奖牌": null},
        {"游戏ID": "zzz", "分路": "中路", "评分": 9.1, "奖牌": "银牌"},
        {"游戏ID": "aaa", "分路": "发育路", "评分": 10.4, "奖牌": "金牌"},
        {"游戏ID": "bbb", "分路": "游走", "评分": 7.7, "奖牌": null}
    ],
    "MVP": {"游戏ID": "aaa", "评分": 10.4}
}"""
...
def result_to_row(res: dict, id2n: dict, ids: list, thr: int, src: str) -> dict | None:
    ...
    if players and isinstance(players[0], str):
        ...
        return {
            ...
            "对抗路选手": names[0], "打野选手": names[1], "中路选手": names[2],
            "发育路选手": names[3], "游走选手": names[4],
            "胜负情况": res.get("比赛结果", "未知"),
            "MVP": f"{mvp_name} {mvp_score}".strip() if mvp_name else "",
            "备注": "",
        }

    # 新格式：含分路/评分/奖牌的选手列表
    lane_map = {}  # {标准分路: {name, score, medal}}
    for p in players[:5]:
        gid = p.get("游戏ID", "")
...
def do_analysis(imgs, auto_frames, api_key, id2n, all_ids, thr, engine="qwen"):
    ...
    rows, errors = [], []
    engine_label = "Qwen-VL-Max"
    prog = st.progress(0, text=f"🤖 {engine_label} 正在分析结算画面...")

    for i, (bts, mime, name) in enumerate(tasks):
        prog.progress((i + 1) / total,
                      text=f"🤖 {engine_label} 正在分析... ({i+1}/{total}) {name}")
        try:
    ...
    if rows:
        ...
        # 显示列（腾讯文档格式）
        display_cols = ["日期", "对抗路选手", "打野选手", "中路选手",
                        "发育路选手", "游走选手", "胜负情况", "MVP", "备注"]
