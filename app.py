...
3. **左侧队伍5名选手**：每人包括【玩家游戏ID（不是英雄名！）】、分路位置、评分
4. **MVP**：获得 MVP 的选手【玩家游戏ID】和评分
5. **奖牌**：识别每个选手的奖牌等级（顶级/金牌/银牌/铜牌）
...
返回格式：
{
    ...
    "比赛日期": "2026/2/6",
    "选手": [
        {"游戏ID": "SPG.xxx", "分路": "对抗路", "评分": 5.6, "奖牌": "金牌"},
        {"游戏ID": "SPG.yyy", "分路": "打野", "评分": 6.3, "奖牌": null},
        {"游戏ID": "某某某", "分路": "中路", "评分": 9.1, "奖牌": "银牌"},
        {"游戏ID": "某某某", "分路": "发育路", "评分": 10.4, "奖牌": "金牌"},
        {"游戏ID": "某某某", "分路": "游走", "评分": 7.7, "奖牌": null}
    ],
...
def match_id(gid: str, id2n: dict, ids: list, thr: int = 60) -> str:
    """将游戏ID模糊匹配为战队成员真名。英雄名过滤，匹配不到则返回原始游戏ID。"""
